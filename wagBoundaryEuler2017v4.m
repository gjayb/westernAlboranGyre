...
% figure; plot(vol); hold all; plot(volF); legend('WAG mass','mass from fluxes')
% figure; plot((volF(1:end-1)-vol.')./vol.'); xlabel('time'); ylabel('% error of mass from fluxes')
...
sfluxv1=zeros([700 200 15 148]);
S2=S-36.5;%!!!!
sfluxh1(2:end-1,2:end-1,:,:)=0.5.*S2(2:end-1,2:end-1,1:15,:).*(rfluxhE(2:end-1,2:end-1,:,:)-abs(rfluxhE(2:end-1,2:end-1,:,:))...
    +rfluxhS(2:end-1,2:end-1,:,:)-abs(rfluxhS(2:end-1,2:end-1,:,:))-rfluxhW(2:end-1,2:end-1,:,:)-abs(rfluxhW(2:end-1,2:end-1,:,:))...
    -rfluxhN(2:end-1,2:end-1,:,:)-abs(rfluxhN(2:end-1,2:end-1,:,:)))...
    +0.5.*S2(1:end-2,2:end-1,1:15,:).*(rfluxhE(2:end-1,2:end-1,:,:)+abs(rfluxhE(2:end-1,2:end-1,:,:)))...
    +0.5.*S2(2:end-1,1:end-2,1:15,:).*(rfluxhS(2:end-1,2:end-1,:,:)+abs(rfluxhS(2:end-1,2:end-1,:,:)))...
    -0.5.*S2(3:end,2:end-1,1:15,:).*(rfluxhW(2:end-1,2:end-1,:,:)-abs(rfluxhW(2:end-1,2:end-1,:,:)))...
    -0.5.*S2(2:end-1,3:end,1:15,:).*(rfluxhN(2:end-1,2:end-1,:,:)-abs(rfluxhN(2:end-1,2:end-1,:,:)));
...
sfluxh=squeeze(sum(sum(sum(sfluxh1))));
sfluxv=squeeze(sum(sum(sum(sfluxv1))));
save('saltAnomAdvWAGeuler2017.mat','sflux*','-v7.3')
...
save('saltAnomWAGeulerDiffusion2017.mat','saltD*');
...
%%
% load('saltAnomAdvWAGeuler2017.mat','sfluxh','sfluxv')
...
% xlabel('Time in days','fontsize',14)
% ylabel('Salt fluxes referenced to salinity 37, kg/s of salt','fontsize',14)
% set(gca,'fontsize',14)
%%
...
% save('heatWAGeuler2017hfw.mat','tflux*','heat*','euler*','-v7.3')
%%
% load('heatWAGeuler2017hfw.mat')
...
load('varying148ts16levelsRho.mat','S');
saltWagEuler=squeeze(sum(sum(sum(repmat(RAC3.*inWag.*dZ3,[1 1 1 148]).*Rho(:,:,1:16,:).*S(:,:,1:16,:)))))+squeeze(sum(sum(ssh148.*squeeze(Rho(:,:,1,:).*S(:,:,1,:)).*repmat(inWag(:,:,1).*RAC,[1 1 148]))));
S2=S-36.5;
saltAnomWagEuler=squeeze(sum(sum(sum(repmat(RAC3.*inWag.*dZ3,[1 1 1 148]).*Rho(:,:,1:16,:).*S2(:,:,1:16,:)))))+squeeze(sum(sum(ssh148.*squeeze(Rho(:,:,1,:).*S2(:,:,1,:)).*repmat(inWag(:,:,1).*RAC,[1 1 148]))));

clear S
load('cp16levels148.mat', 'cp')
load('varying148ts16levelsRho.mat','T');
T=T+273.15;%kelvin!
heatWagEuler=squeeze(sum(sum(sum(repmat(RAC3.*inWag.*dZ3,[1 1 1 148]).*Rho(:,:,1:16,:).*cp(:,:,1:16,:).*T(:,:,1:16,:)))))+squeeze(sum(sum(ssh148.*squeeze(Rho(:,:,1,:).*cp(:,:,1,:).*T(:,:,1,:)).*repmat(inWag(:,:,1).*RAC,[1 1 148]))));
