...
plot(-sum(tendency,1)+sum(windstress,1)+sum(diffbot,1)-sum(difftop(2:6,:),1)+sum(advection,1)+sum(dissipation,1)+sum(pressClin,1)+sum(timestep,1),'g--','linewidth',2)
...
legend('-d/dt','wind + v. diffusion','advection','drag + h. diffusion','total')
title('Depth-integrated vorticity budget, 185m, Euler WAG','fontsize',14)
xlabel('days','fontsize',12)
ylabel('m^3/s^2, negative speeds up gyre','fontsize',12)
set(gca,'fontsize',12)
