...
for j=21;%58:68 %0:0.5:21
    j
    clear *tr*
    tmesh=86400*(j):43200:(j+2)*86400;
    ...
    lattr2A=ones(size(ytr2A)).*ymin+ytr2A./111000;
     ...
     disp('save A')
   fn=strcat('wagLobeDay',num2str(j),'A2day.mat');
   save(fn,'-v7.3')
    figure
    plot(lonCoast,latCoast,'k')
    hold on
    plot(lontrA(end,:),lattrA(end,:),'ro')
    plot(lontr2A(end,:),lattr2A(end,:),'bo')
    plot(xc1,yc1,'k.')
    legend('coast','forward','backward','initial')
    title(strcat('Lobe 2 days forward and backward from day ',num2str(j)))
    fn=strcat('wagLobeDay',num2str(j),'A2day.pdf');
    save2pdf(fn)
    
    %%
    tmesh=86400*(j+2):43200:(j+4)*86400;
    tmesh2=(j-2)*86400:-43200:(j-4)*86400;
    %options=odeset('RelTol',10^(-10),'AbsTol',10^(-13));
   ...
   %save(fn,'-v7.3')
    ...
    hold on; plot(xc1,yc1,'k.')
    hold on; plot(lonCoast,latCoast,'k')
    legend('forward','backward','initial','coast')
    title(strcat('Lobe forward and backward 4 days from day',num2str(j)))
    fn=strcat('wagLobeDay',num2str(j),'B2day.pdf');
    save2pdf(fn)
    %%
        tmesh=86400*(j+4):86400:(j+6)*86400;
    tmesh2=(j-4)*86400:-86400:(j-6)*86400;
%     %options=odeset('RelTol',10^(-10),'AbsTol',10^(-13));
%     
...
%     
   ...
   %save(fn,'-v7.3')
    ...
    plot(xc1,yc1,'k.')
    title(strcat('Lobes forward and backward 6 days from day ',num2str(j)))
    fn=strcat('wagLobeDay',num2str(j),'C2day.pdf');
    save2pdf(fn)
    
 %%   
        tmesh=86400*(j+6):86400:(j+8)*86400;
    tmesh2=(j-6)*86400:-86400:(j-8)*86400;
%     %options=odeset('RelTol',10^(-10),'AbsTol',10^(-13));
%     
   ...
   fn=strcat('wagLobeDay',num2str(j),'D2day.mat');
   save(fn,'-v7.3')
    figure
    plot(lonCoast,latCoast,'k')
    hold on
    ...
    plot(xc1,yc1,'k.')
    legend('coast','forward','backward','initial')
    title(strcat('Lobe 8 day integration day',num2str(j)))
    fn=strcat('wagLobeDay',num2str(j),'D2day.pdf');
    save2pdf(fn)
