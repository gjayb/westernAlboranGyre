...
times=8640:8640:(59*8640);
days=1:59;
times2=[1 8640:8640:(59*8640)];
% 
...
%%
load('edgesWAGeuler2017NR.mat','inWag')
load('tsTendsnap.mat')
load('fluxesOtherT59day.mat','Ttend')
load('fluxesOtherS59day.mat','Stend')
...
% load('edgesWAGeuler2017NR.mat','inWag','open*','*edge','XC','YC','*edge','*Coast')
...
% days=1:148;
% Rho=999.8*ones([700 200 148]);%squeeze(Rho(:,:,1,days));
...
% surfQ=squeeze(nansum(nansum(Tflux.*repmat(inWag(:,:,1).*cellVol(:,:,1),[1 1 nt])./(dZ(1,1,1).*repmat(hFacC(:,:,1),[1 1 nt]).*Rho.*cp))));
% dTdt=squeeze(nansum(nansum(nansum(Ttend.*cellVol4.*repmat(inWag,[1 1 1 nt])/86400))));
% surfQMag=squeeze(nansum(nansum(abs(Tflux).*repmat(inWag(:,:,1).*cellVol(:,:,1),[1 1 nt])./(dZ(1,1,1).*repmat(hFacC(:,:,1),[1 1 nt]).*Rho.*cp))));
% dTdtMag=squeeze(nansum(nansum(nansum(abs(Ttend).*cellVol4.*repmat(inWag,[1 1 1 nt])/86400))));
% surfQnet=squeeze(nansum(nansum(Qnet.*repmat(inWag(:,:,1).*cellVol(:,:,1),[1 1 nt])./(dZ(1,1,1).*repmat(hFacC(:,:,1),[1 1 nt]).*Rho.*cp))));%tflux=qnet+qprecipevap
...
% clear open* *edge
% save('temperatureBudgetEulerWAG148dayNR1v7.mat')%,'-v7.3')%only want vectors
% %% old junk
% % advTh=squeeze(nansum(nansum(nansum((AdvUt(1:end-1,1:end-1,:,:)-AdvUt(2:end,1:end-1,:,:)+AdvVt(1:end-1,1:end-1,:,:)...
...
% figure; plot(advTHopenS); hold all
% plot(advTBopen); plot(difTHopenS); %plot(difZopen)
% plot(difTZopen); plot(surfaceT); plot(-dTdt)
% plot(advTHopenS+advTBopen+difTHopenS+difTZopen+surfaceT-dTdt,'--','linewidth',2)
% xlabel('days'); ylabel('potential temperature flux, \circ C m^3/s')
% legend('horizontal advection','vertical advection','horizontal diffusion','vertical diffusion','surface','dT/dt','sum(fluxes)')
% title('temperature budget, Euler WAG')
% 
% figure; plot(advTHopenS+advTBopen,'linewidth',2); hold all %plot(advTBopen);
%  plot(difTHopenS,'linewidth',2); %plot(difZopen)
% plot(difTZopen,'linewidth',2); plot(surfaceT,'linewidth',2); plot(-dTdt,'linewidth',2)
% plot(advTHopenS+advTBopen+difTHopenS+difTZopen+surfaceT-dTdt,'--','linewidth',2)
% xlabel('days'); ylabel('potential temperature flux, \circ C m^3/s')
% legend('advection','horizontal diffusion','vertical diffusion','surface','dT/dt','sum(fluxes)')
% title('temperature budget, Euler WAG')
% 
% figure; plot(advTHopenSMag); hold all
% plot(advTBopenMag); plot(difTHopenSMag); %plot(difZopen)
% plot(difTZopen1Mag+difTZopen2Mag); plot(surfTtend3Mag.'+abs(surfaceT)); plot(dTdtMag)
% title('Temperature flux magnitudes, Euler WAG')
% xlabel('days'); ylabel('potential temperature flux, \circ C m^3/s')
% legend('horizontal advection','vertical advection','horizontal diffusion','vertical diffusion','surface','dT/dt')
% 
% meanTermsC={'adv','h diff','z diff','surface','-dTdt','total'};
% meanTerms=[mean(advTHopenS+advTBopen),mean(difTHopenS),mean(difTZopen),mean(surfaceT),mean(-dTdt),mean(advTHopenS+advTBopen+difTHopenS+difTZopen+surfaceT-dTdt)];
% figure; bar(meanTerms)
% set(gca,'XTickLabel',meanTermsC)
% title('Mean Euler WAG Temperature Fluxes')
% 
% meanTermsC={'adv side','adv bottom','h diff','z diff','surface','dTdt'};
% meanTerms=[mean(advTHopenSMag),mean(advTBopenMag),mean(difTHopenSMag),mean(difTZopen1Mag+difTZopen2Mag),mean(surfTtend3Mag.'+abs(surfaceT)),mean(dTdtMag)];
% figure; bar(meanTerms)
% set(gca,'XTickLabel',meanTermsC)
% title('Mean Euler WAG Temperature Flux Magnitudes')
% 
...
% load('edgesWAGeuler2017NR.mat','inWag','open*','*edge','XC','YC')
...
% save('salinityBudgetEulerWag148daysIntegralv7.mat')%,'-v7.3')
% %% plots
% %difSaltBopen=difSaltBopen1+difSaltBopen2;
% 
% figure;
% plot(advSHopenS,'linewidth',2); hold all 
% plot(advSBopen,'linewidth',2)
...
% xlabel('days')
% ylabel('integrated changes in salinity, psu m^3/s')
...
% plot(difSZopen1Mag+difSZopen2Mag,'linewidth',2); plot(surfStend3Mag.'+surfFMag,'linewidth',2)
% plot(dSdtMag,'linewidth',2)
% legend('advection through sides','advection through bottom','diffusion through sides','diffusion through bottom','surface','-dS/dt','sum')
% xlabel('days')
% ylabel('magnitude of salinity fluxes, psu m^3/s')
% title('Euler salinity budget flux magnitudes')
% 
% figure;
...
% % plot(-dSdt,'linewidth',2)
% % plot(sumflux-dSdt-reS,'--','linewidth',4)
...
% meanTerms=[mean(advSHopenS+advSBopen),mean(difSHopen),mean(difSZopen),mean(surfStend3.'+surfF),mean(-dSdt),mean(advSHopenS+advSBopen+difSHopen+difSZopen+surfStend3.'+surfF-dSdt)];
% figure; bar(meanTerms)
% set(gca,'XTickLabel',meanTermsC)
% title('Mean Euler WAG Salinity Fluxes')
% 
% meanTermsC={'adv','h diff','z diff','surface','-dSdt','total'};
% meanTerms=[mean(advSHopenSMag),mean(advSBopenMag),mean(difSHopenMag),mean(difSZopen1Mag+difSZopen2Mag),mean(surfStend3Mag.'+surfFMag),mean(dSdtMag)];
% figure; bar(meanTerms)
% set(gca,'XTickLabel',meanTermsC)
% title('Mean Euler WAG Salinity Fluxes')
% 
