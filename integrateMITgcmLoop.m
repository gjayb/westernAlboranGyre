...
load('uvwIso295interpolated.mat');
%load('uvAdepth1interpolated.mat');
addpath('../mStuff')
clear w
[~,~,nt]=size(u);

for intLoop=[22 61 104 130]%115 %:1:34
    tmesh=(intLoop)*86400:7200:(intLoop+8)*86400;
    tmesh2=(i ntLoop)*86400:-7200:(intLoop-8)*86400; 
    intLoop
...
axis([-6 0 34 38])
save2pdf('forwardtraj8daySurfaceDay115.pdf')
close all     
...
axis([-6 0 34 38])
save2pdf('backwardtraj8daySurfaceDay115.pdf')
close all    
%%axis([-6 0 34 38])
...
disp('saving')
fname='/nobackup1/gbrett/traj8dayIso295Day115c.mat';
save(fname,'-v7.3')
disp('done all')
